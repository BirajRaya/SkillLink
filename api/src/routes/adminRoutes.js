  ...
  // Special route for updating profile pictures separately
  router.post('/users/:id/profile-picture', upload.single('profilePicture'), async (req, res) => {
    logAction('received profile picture update', { userId: req.params.id });
    ...
    try {
      ...
      const updatedUser = await AdminUserService.updateUserProfilePicture(userId, profilePicture);
      
      logAction('updated profile picture', { userId });
      ...
      logAction('error updating profile picture', { 
  ...
  // Get all users route - optimized to not include profile pictures
  router.get('/users', async (req, res) => {
    try {
      // Use new method that returns users without full profile pictures
      const users = await AdminUserService.getAllUsersWithoutPictures();
      res.json(users);
    } catch (error) {
      res.status(500).json({ 
  ...
  router.get('/users/:id/profile-picture', async (req, res) => {
    try {
      const userId = req.params.id;
      const profilePicture = await AdminUserService.getUserProfilePicture(userId);
  ...
  // UPDATE user route - handling regular fields, profile picture is updated separately
